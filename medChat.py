...
def get_answer(question: str, knowledge: dict) -> str | None:
    for q in knowledge["diseases"]:
        if q["disease"] == question:
            return q["medicine"]
...
def chat_bot():
    ...
    while True:
        ...
        bestmatch: str | None = best_match(ip, [q["disease"] for q in knowledge["diseases"]])
        ...
        if bestmatch:
            ...
            if new_ans.lower() != 's':
                ...
                save_knowledge('meds.json', knowledge)
